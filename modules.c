...
// ---------------------------------------------------------------------
AddNodeToGroupSoft(struct group *g, int node)
{
  struct node_lis *p = g->nodeList;
  int totlink,inlink;
...
int RemoveNodeFromGroup(struct group *g, struct node_gra *node)
{
  ...
  if (p->next == NULL)
    ...
    totweight = CountLinksWeight(node);
    inweight = CountLinksWeightInGroup(node,g);
...
{
  struct group *CopyPartition(struct group *);
  struct group *copy;
...
struct node_gra *BuildNetFromPart(struct group *part)
{
  ...
  while(p->next != NULL){
    ...
    new=CreateNodeGraph(last,p->ref->num,p->ref->pos_x,p->ref->pos_y);
