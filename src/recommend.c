...
*/
int
GetDecorrelationStep2StateFast(double *H,
...
*/
void
ThermalizeMC2StateFast(int decorStep,
		       double *H,
		       struct node_gra **nlist1, struct node_gra **nlist2,
		       struct group **glist1, struct group **glist2,
		       struct group *part1, struct group *part2,
...
*/
double *
MultiLinkScore2StateFast(struct binet *ratings,
			 struct query **querySet, int nquery,
			 int nIter,
			 gsl_rng *gen,
			 char verbose_sw,
  ...
  */
  ...
  for (iter=0; iter<nIter; iter++) {
    ...
    case 'v':
      ...
      break;
    case 'd':
      fprintf(stderr, "%d %lf %lf\n", iter, H, H2StateFast(part1, part2));
      FPrintPartition(stderr, part1, 0);
      FPrintPartition(stderr, part2, 0);
      break;
    }
